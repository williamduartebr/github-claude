    ...
    <div class="bg-white rounded-b-lg border-l border-r border-b border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                {{-- Ajuste na seção da tabela para exibir dados adicionais quando disponíveis --}}

                <!-- Dentro da tabela, após as colunas existentes, adicionar condicional para dados extras -->
                <tbody class="divide-y divide-gray-200">
                    @foreach($tireSpecsByVersion as $index => $spec)
                    <tr
                        class="hover:bg-gray-50 transition-colors duration-200 {{ ($spec['is_main_version'] ?? $index === 0) ? 'bg-blue-50' : '' }}">
                        <!-- Versão (coluna existente) -->
                    ...
                    @else bg-gray-100 text-gray-800 @endif">
                            ...
                            {{-- NOVO: Exibir informações extras quando disponíveis --}}
                            @if(!empty($spec['engine_info']) || !empty($spec['transmission']) || !empty($spec['power']))
                            <div class="mt-2 space-y-1">


                                @if(!empty($spec['transmission']))
                                <div class="text-xs text-gray-600 flex items-center">
                                    <span class="inline-block w-2 h-2 bg-green-400 rounded-full mr-1"></span>
                                    Câmbio: {{ $spec['transmission'] }}
                                </div>
                                @endif
                            </div>
                            @endif
                        ...
                        <td class="py-4 px-4 text-sm text-center">
                            <div class="space-y-2">
                                ...
                                @if(!empty($spec['load_speed_index']) && $spec['load_speed_index'] !== 'Consulte
                                manual')
            ...
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-sm text-red-700 mt-1">
                        Para track days ou uso esportivo, considere reduzir 2-3 PSI para pneus frios. Sempre aqueça os
                        pneus gradualmente.
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-sm text-green-700 mt-1">
                        Para uso off-road, reduza 3-5 PSI do traseiro para melhor tração na terra. Recalibre para
                        asfalto.
                    </p>
                </div>
            </div>
