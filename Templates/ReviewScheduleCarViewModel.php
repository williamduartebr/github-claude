...
class ReviewScheduleCarViewModel extends TemplateViewModel
{
    ...
    private function processStructuredDataForSEO(): void
    {
        ...
        // CORREÇÃO: Usar $this->article ao invés de $article (variável inexistente)
        // CORREÇÃO: Usar tipo_veiculo do extracted_entities
        ...
        if (!empty($vehicleInfo['marca']) && !empty($vehicleInfo['modelo'])) {
            ...
            $schemaVehicleType = ($vehicleInfo['tipo_veiculo'] ?? '') === 'motocicleta' ? 'Motorcycle' : 'Vehicle';
            ...
            $structuredData['mainEntity'] = [
                ...
                'totalTime' => 'PT2H', // Tempo estimado para revisão
                'estimatedCost' => [
                    '@type' => 'MonetaryAmount',
                    'currency' => 'BRL',
    ...
    private function extractAverageCost(array $overview): string
    {
        ...
        return empty($costs) ? '500' : (string) intval(array_sum($costs) / count($costs));
    }
