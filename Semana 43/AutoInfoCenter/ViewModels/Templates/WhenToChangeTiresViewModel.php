...
class WhenToChangeTiresViewModel extends TemplateViewModel
{
    ...
    private function processStructuredDataForSEO(): void
    {
        ...
        if (!empty($vehicleInfo['marca']) && !empty($vehicleInfo['modelo'])) {
            ...
            // Apenas adiciona informações como keywords
            $vehicleKeywords = [
                $vehicleInfo['marca'] ?? '',
                $vehicleInfo['modelo'] ?? '',
                $vehicleInfo['ano'] ?? '',
            ...
            $allKeywords = array_merge(
                ['quando trocar pneus', 'sinais desgaste', 'manutenção pneus'],
            ...
            if (!empty($vehicleName)) {
                $structuredData['description'] = $structuredData['description'] .
                    ' Específico para ' . $vehicleName . '.';
            }

            // Adiciona HowTo como mentions (não mainEntity) para evitar erro
            $structuredData['mentions'] = [
                '@type' => 'HowTo',
                'name' => 'Como Verificar Quando Trocar os Pneus',
                'description' => 'Guia passo a passo para verificar sinais de desgaste',
                'totalTime' => 'PT15M',
