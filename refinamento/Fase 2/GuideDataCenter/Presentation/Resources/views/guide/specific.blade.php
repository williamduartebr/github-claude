    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-wrap items-center gap-3 mb-4">
            @foreach($badges as $badge)
                ...
                @endphp
                <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border border-gray-200 {{ $colorClass }}">
                    {{ $badge['text'] }}
                </div>
            @endforeach
        </div>
    ...
    <section class="bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-8">
        ...
        <div class="mt-4 p-3 bg-blue-50 border-l-4 border-blue-500 rounded-r">
            <p class="text-xs text-blue-800 font-roboto">
                💡 Todos os óleos acima atendem ou superam a especificação mínima exigida para o {{ $make['name'] }} {{ $model['name'] }} {{ $year }}.
            </p>
        </div>
    </section>
