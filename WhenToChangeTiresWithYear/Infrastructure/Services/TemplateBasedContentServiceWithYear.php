...
class TemplateBasedContentServiceWithYear
{
    ...
    protected function generateRelatedContentSuggestions(VehicleData $vehicle): array
    {
        ...
        $suggestions[] = [
            'title' => "Manutenção Preventiva do {$vehicle->make} {$vehicle->model} {$vehicle->year}: Checklist Completo",
            'slug' => "manutencao-preventiva-" . Str::slug("{$vehicle->make}-{$vehicle->model}-{$vehicle->year}"),
            'icon' => 'wrench'
        ];
        ...
        if (!$vehicle->isElectric()) {
            $suggestions[] = [
                'title' => "Como Economizar Combustível no {$vehicle->make} {$vehicle->model} {$vehicle->year}",
                'slug' => "como-economizar-combustivel-" . Str::slug("{$vehicle->make}-{$vehicle->model}-{$vehicle->year}"),
                'icon' => 'gas-pump'
            ];
        }
