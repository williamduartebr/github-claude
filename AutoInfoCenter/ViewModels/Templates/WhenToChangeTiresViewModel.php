...
class WhenToChangeTiresViewModel extends TemplateViewModel
{
    ...
    private function processStructuredDataForSEO(): void
    {
        ...
        // Constrói informações estruturadas para manutenção de pneus
        $this->processedData['structured_data'] = [
            ...
            'about' => [
                ...
                'vehicleModelDate' => $vehicleInfo['year'] ?? ''
            ]
        ];
    ...
    private function processPreventiveMaintenance(array $maintenance): array
    {
        ...
        foreach ($maintenance as $key => $item) {
            if ($key === 'cuidados_gerais' && is_array($item)) {
                $processed['general_care'] = $item;
            } elseif (is_array($item) && !empty($item['frequencia'])) {
                $processed['tasks'][] = [
                    'key' => $key,
