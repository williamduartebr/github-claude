...
@php
    ...
    if ($articles->count() > 0) {
        $collectionSchema['hasPart'] = $articles->take(20)->map(function($article) use ($category) {
            return [
                '@type' => 'Article',
    ...
    if ($articles->count() > 0) {
        $collectionSchema['mainEntity'] = [
            ...
            'itemListElement' => $articles->take(20)->map(function($article, $index) {
                return [
                    '@type' => 'ListItem',
                    'position' => $index + 1,
                    'url' => url('/info/' . $article->slug)
    ...
    // Schema FAQ específico para All Models
    $faqSchema = [
        '@context' => 'https://schema.org',
        '@type' => 'FAQPage',
        'mainEntity' => [
