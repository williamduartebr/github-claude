 ...
 * 
 * FASE 2 do processo TireCalibration:
...
class ArticleGenerationService
{
     ...
     */
    public function generateCalibrationArticle(TireCalibration $calibration): array
    {
        try {
     ...
     */
    private function validateCalibrationData(TireCalibration $calibration): void
    {
        ...
        if ($calibration->enrichment_phase !== TireCalibration::PHASE_VEHICLE_ENRICHED) {
            throw new \Exception("TireCalibration não está na fase correta. Fase atual: {$calibration->enrichment_phase}");
        }
     ...
     */
    private function extractProcessedVehicleData(TireCalibration $calibration): array
    {
     ...
     */
    private function extractProcessedPressureData(TireCalibration $calibration): array
    {
     ...
     */
    private function extractTechnicalSpecs(TireCalibration $calibration): array
    {
    ...
    public function validateMappedArticle(array $article): array
    {
        ...
        if (isset($article['generation_metadata']['source_data_quality_score']) && 
    ...
    {
        $total = TireCalibration::where('enrichment_phase', TireCalibration::PHASE_VEHICLE_ENRICHED)->count();
        $mapped = TireCalibration::whereNotNull('generated_article')->count();
        $avgQuality = TireCalibration::whereNotNull('content_quality_score')->avg('content_quality_score') ?? 0;
