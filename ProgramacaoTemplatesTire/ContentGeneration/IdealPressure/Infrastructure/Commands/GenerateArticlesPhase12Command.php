 ...
 * 
 * Command principal do módulo IdealPressure que executa:
 ...
 * 
 * USO:
 * php artisan ideal-pressure:generate-articles
 * php artisan ideal-pressure:generate-articles --limit=10 --dry-run
 * php artisan ideal-pressure:generate-articles --category=sedan --force
 * 
 * @author Claude Sonnet 4
 * @version 1.0
 */
class GenerateArticlesPhase12Command extends Command
{
    /**
     * The name and signature of the console command.
     */
    protected $signature = 'ideal-pressure:generate-articles
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            // 2. Buscar IdealPressure candidates
            $candidates = $this->getCandidateCalibrations($config);

            if ($candidates->isEmpty()) {
                $this->warn('❌ Nenhuma IdealPressure encontrada com os critérios especificados.');
                return self::SUCCESS;
            }

            $this->info("📊 Encontradas {$candidates->count()} IdealPressure(s) para processamento");
            $this->newLine();

            // 3. Processar IdealPressures
    ...
    /**
     * Buscar IdealPressures candidatas para processamento
     */
    private function getCandidateCalibrations(array $config)
    {
        $query = IdealPressure::query()
            ->where('enrichment_phase', IdealPressure::PHASE_VEHICLE_ENRICHED)
        ...
        foreach ($candidates as $calibration) {
            ...
            try {
                ...
                if (!$config['dry_run']) {
                    ...
                    $calibration->enrichment_phase = IdealPressure::PHASE_ARTICLE_GENERATED;
                    $calibration->last_processing_at = now();
                    $calibration->processing_history = array_merge(
                        $calibration->processing_history ?? [],
                        [[
                            'phase' => IdealPressure::PHASE_ARTICLE_GENERATED,
    ...
    /**
     * Validar pré-requisitos específicos da IdealPressure
     */
    private function validateCalibrationPrerequisites(IdealPressure $calibration): bool
    {
        ...
        if (empty($calibration->vehicle_basic_data) && empty($calibration->pressure_specifications)) {
            Log::warning('GenerateArticlesPhase1Command: IdealPressure sem dados VehicleData mapeados', [
        ...
        if ($calibration->enrichment_phase !== IdealPressure::PHASE_VEHICLE_ENRICHED) {
            Log::warning('GenerateArticlesPhase1Command: IdealPressure em fase incorreta', [
                'tire_calibration_id' => $calibration->_id,
                'current_phase' => $calibration->enrichment_phase,
                'expected_phase' => IdealPressure::PHASE_VEHICLE_ENRICHED
            ]);
            return false;
        }
    ...
    private function displayFinalStats(array $results, float $duration): void
    {
        ...
        if ($results['success'] > 0) {
            ...
            $this->line('   1. Execute: php artisan ideal-pressure:refine-with-claude');
            $this->line('   2. Para estatísticas: php artisan ideal-pressure:stats');
            $this->newLine();
        }
        ...
        if ($results['skipped'] > 0) {
            ...
            $this->line('   • IdealPressure em fase incorreta');
    ...
    {
        return [
            'ready_for_processing' => IdealPressure::where('enrichment_phase', IdealPressure::PHASE_VEHICLE_ENRICHED)->count(),
            'articles_generated' => IdealPressure::where('enrichment_phase', IdealPressure::PHASE_ARTICLE_GENERATED)->count(),
            'failed_processing' => IdealPressure::where('enrichment_phase', IdealPressure::PHASE_FAILED)->count(),
            'categories_available' => IdealPressure::distinct('main_category')->pluck('main_category')->filter()->values(),
            'avg_quality_score' => round(IdealPressure::whereNotNull('data_completeness_score')->avg('data_completeness_score'), 1),
