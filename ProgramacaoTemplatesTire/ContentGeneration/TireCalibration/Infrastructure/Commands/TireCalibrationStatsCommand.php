...
/**
 * TireCalibrationStatsCommand - Estatísticas completas do módulo TireCalibration
 * 
 ...
 * 
 * USO:
 * php artisan tire-calibration:stats
 * php artisan tire-calibration:stats --detailed
 * php artisan tire-calibration:stats --export=csv
 * php artisan tire-calibration:stats --category=sedan
 * 
 * @author Claude Sonnet 4
 * @version 1.0
 */
class TireCalibrationStatsCommand extends Command
{
    /**
     * The name and signature of the console command.
     */
    protected $signature = 'tire-calibration:stats
     ...
     */
    protected $description = 'Exibir estatísticas completas do módulo TireCalibration';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            Log::info('TireCalibrationStatsCommand: Estatísticas coletadas', [
            ...
            Log::error('TireCalibrationStatsCommand: Erro fatal', [
                'error' => $e->getMessage(),
    ...
    private function getPhaseStats(array $config): array
    {
        ...
        $phases = [
            TireCalibration::PHASE_PENDING => 'Pendente',
            TireCalibration::PHASE_VEHICLE_ENRICHED => 'VehicleData Processado',
            TireCalibration::PHASE_ARTICLE_GENERATED => 'Artigo Gerado (Fase 1+2)',
            TireCalibration::PHASE_CLAUDE_PROCESSING => 'Processando Claude',
            TireCalibration::PHASE_CLAUDE_COMPLETED => 'Refinado (Fase 3)',
            TireCalibration::PHASE_FAILED => 'Falhou',
        ];
    ...
    private function getCategoryStats(array $config): array
    {
        ...
        foreach ($categories as $category => $count) {
            ...
            $categoryQuery = TireCalibration::where('main_category', $category);
            
            $categoryStats[$category] = [
                'total' => $count,
                'percentage' => $percentage,
                'completed' => $categoryQuery->where('enrichment_phase', TireCalibration::PHASE_CLAUDE_COMPLETED)->count(),
                'failed' => $categoryQuery->where('enrichment_phase', TireCalibration::PHASE_FAILED)->count(),
    ...
    {
        $query = TireCalibration::query()->whereNotNull('data_completeness_score');
    ...
    private function getTemporalStats(array $config): array
    {
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $dailyStats[$date->format('Y-m-d')] = [
                ...
                'completed' => $dayQuery->where('enrichment_phase', TireCalibration::PHASE_CLAUDE_COMPLETED)->count(),
                'failed' => $dayQuery->where('enrichment_phase', TireCalibration::PHASE_FAILED)->count(),
            ];
        }
    ...
    private function displayRecommendations(array $stats): void
    {
        ...
        $this->line('   1. Monitorar pipeline: php artisan tire-calibration:stats --detailed');
        $this->line('   2. Processar pendências: php artisan tire-calibration:generate-articles');
        $this->line('   3. Refinar artigos: php artisan tire-calibration:refine-with-claude --limit=10');
    }
    ...
    {
        $icons = [
            TireCalibration::PHASE_PENDING => '⏳',
            TireCalibration::PHASE_VEHICLE_ENRICHED => '✅',
            TireCalibration::PHASE_ARTICLE_GENERATED => '📝',
            TireCalibration::PHASE_CLAUDE_PROCESSING => '🤖',
            TireCalibration::PHASE_CLAUDE_COMPLETED => '🎯',
            TireCalibration::PHASE_FAILED => '❌',
        ];
    ...
    {
        $completed = $phaseStats[TireCalibration::PHASE_CLAUDE_COMPLETED]['count'] ?? 0;
    ...
    {
        $total = TireCalibration::where('main_category', $category)->count();
        $success = TireCalibration::where('main_category', $category)
            ->where('enrichment_phase', TireCalibration::PHASE_CLAUDE_COMPLETED)->count();
    ...
    {
        $completed = $query->where('enrichment_phase', TireCalibration::PHASE_CLAUDE_COMPLETED)
    ...
    {
        $total = $query->count();
        $failed = (clone $query)->where('enrichment_phase', TireCalibration::PHASE_FAILED)->count();
