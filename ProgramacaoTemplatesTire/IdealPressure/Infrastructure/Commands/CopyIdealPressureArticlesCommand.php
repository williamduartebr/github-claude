...
/**
 * CopyIdealPressureArticlesCommand - CORRIGIDO - Versões V1/V2
 * 
 * V1: Inclui vehicle_year (963 artigos esperados)
 * V2: Remove vehicle_year (300+ artigos esperados)
 * 
 * USO:
 * php artisan ideal-pressure:copy-calibration --versao=v1 --limit=100 --dry-run
 * php artisan ideal-pressure:copy-calibration --versao=v2 --validate
 * php artisan ideal-pressure:copy-calibration --versao=both --force
 * 
...
class CopyIdealPressureArticlesCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->info('🔄 COPIANDO ARTIGOS IDEAL PRESSURE - VERSÕES V1/V2');
        ...
        try {
            ...
            if ($articles->isEmpty()) {
                $this->warn('⚠️ Nenhum artigo encontrado com template_type = ideal');
                return self::SUCCESS;
            }

            $this->info("📊 {$articles->count()} artigos ideal encontrados para processamento");
            $this->newLine();
    ...
    protected function displayConfiguration(array $config): void
    {
        ...
        $this->line("   • V2: SEM vehicle_year (~300+ artigos)");
        $this->newLine();
    }
    ...
    protected function generateGenericUrlForV2(array $vehicleData): string
    {
        ...
        return "pressao-ideal-pneu-{$make}-{$model}";
    }
    ...
    protected function displayFinalStats(float $startTime): void
    {
        ...
        Log::info('CopyIdealPressureArticlesCommand: Execução concluída', [
