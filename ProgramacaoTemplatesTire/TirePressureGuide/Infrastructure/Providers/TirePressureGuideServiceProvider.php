...
use Src\ContentGeneration\TirePressureGuide\Infrastructure\Console\Commands\CheckIndexesCommand;
use Src\ContentGeneration\TirePressureGuide\Infrastructure\Services\VehicleDataProcessorService;
use Src\ContentGeneration\TirePressureGuide\Infrastructure\Console\Commands\ManageIndexesCommand;
use Src\ContentGeneration\TirePressureGuide\Infrastructure\Services\VehicleDataCorrectionService;
use Src\ContentGeneration\TirePressureGuide\Infrastructure\Console\Commands\FixTemplateUsedCommand;
...
use Src\ContentGeneration\TirePressureGuide\Infrastructure\Console\Commands\Schedules\TirePressureRefinementSchedule;
...
class TirePressureGuideServiceProvider extends ServiceProvider
{
    ...
    protected array $commands = [
        ...
        FixTemplateUsedCommand::class,

        // 2 fase
        // TirePressureRefinementSchedule::class,
    ...
    protected function registerCommandAliases(): void
    {
        if ($this->app->runningInConsole()) {
            ...
            $this->app->singleton(
                'command.tire-pressure.generate',
                GenerateTirePressureArticlesCommand::class
            );

            $this->app->singleton(
                'command.tire-pressure.publish',
                PublishTirePressureArticlesCommand::class
            );

            $this->app->singleton(
                'command.tire-pressure.publish-temp',
                PublishTempTirePressureArticlesCommand::class
            );

            // ✅ NOVOS ALIASES
            $this->app->singleton(
                'command.tire-pressure.correct-vehicle-data',
                VehicleDataCorrectionSchedule::class
            );
