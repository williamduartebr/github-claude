{{--
Template AMP: ideal_tire_pressure_pickup.blade.php
ADAPTADO PARA PICKUPS - MANTÉM TODO CONTEÚDO dos mocks, apenas incrementa funcionalidades específicas
--}}
...
<div class="container">
    <article>
        ...
        <!-- PRESSÕES PRINCIPAIS PARA PICKUP - ADAPTADO -->
        <div class="highlight-box">
            <div class="highlight-title">🚛 Pressões Ideais para {{ $vehicleName }}</div>
            <!-- Layout adaptado para pickup com 3 colunas quando há carga -->
            ...
            <div class="pickup-pressure-grid">
                ...
                <div class="pressure-card loaded">
                    ...
                    <div class="pressure-value">{{ $pressureSpecs['loaded_pressure_display'] ?? $pressureSpecs['pressure_max_front'].'/'.$pressureSpecs['pressure_max_rear'] }}</div>
                    <div class="pressure-unit">PSI</div>
                ...
                <div class="version-card">
                    ...
                    <div class="version-body">
                        ...
                        <div class="spec-row">
                            ...
                            <span class="spec-value">{{ $spec['front_normal'] ?? $pressureSpecs['pressure_empty_front'] ?? '35' }} PSI</span>
                        </div>
                        <div class="spec-row">
                            <span class="spec-label">Traseiro (Normal):</span>
                            <span class="spec-value">{{ $spec['rear_normal'] ?? $pressureSpecs['pressure_empty_rear'] ?? '40' }} PSI</span>
                        </div>
                        <div class="spec-row">
                            <span class="spec-label">Dianteiro (c/ Carga):</span>
                            <span class="spec-value loaded">{{ $spec['front_loaded'] ?? $pressureSpecs['pressure_max_front'] ?? '38' }} PSI</span>
                        </div>
                        <div class="spec-row">
                            <span class="spec-label">Traseiro (c/ Carga):</span>
                            <span class="spec-value loaded">{{ $spec['rear_loaded'] ?? $pressureSpecs['pressure_max_rear'] ?? '45' }} PSI</span>
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        <!-- TABELA CARGA COMPLETA PARA PICKUP -->
        @if(!empty($fullLoadTable['conditions']))
        <div class="content-section">
            <div class="section-header">
                <span class="section-icon">📊</span>
                <span class="section-title">{{ $fullLoadTable['title'] ?? 'Pressões para Carga na Caçamba' }}</span>
            </div>
                ...
                <table class="load-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $condition['occupants'] ?? '2-5' }}</td>
                            <td>{{ $condition['luggage'] ?? $condition['baggage'] ?? 'Normal' }}</td>
                            <td><span class="pressure-badge">{{ $condition['front_pressure'] ?? $condition['pressure_front'] ?? '35' }}</span></td>
                            <td><span class="pressure-badge {{ str_contains($condition['front_pressure'] ?? '', '4') ? 'loaded' : '' }}">{{ $condition['rear_pressure'] ?? $condition['pressure_rear'] ?? '40' }}</span></td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="alert-box alert-info" style="margin-top: 12px;">
                <p style="margin: 0; font-size: 14px;"><strong>💡 Dica:</strong> Use pressões "Normal" para uso urbano sem carga. Use pressões maiores quando transportar peso na caçamba ou rebocar.</p>
            </div>
        </div>
        @endif
        ...
        <!-- CONDIÇÕES ESPECIAIS PARA PICKUPS - MELHORADA -->
                ...
                <div class="condition-card">
                    ...
                    <div class="condition-body">
                        <div class="condition-adjustment">
                            ...
                            <div class="adjustment-value">{{ $condition['recommended_adjustment'] ?? '' }}</div>
                        </div>
        ...
        <div class="content-section">
            ...
            <div class="alert-box alert-info" style="margin-bottom: 16px;">
                ...
                @elseif(!empty($dica['procedures']) && is_array($dica['procedures']))
                <ul style="margin: 8px 0 0 16px; font-size: 14px;">
            ...
            <div style="text-align: left; max-width: 600px; margin: 0 auto;">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    ...
                    <div>
                        ...
                        <p style="margin: 0; font-size: 14px;">
                            Dianteiros: <strong>{{ $pressureSpecs['pressure_max_front'] ?? $pressureSpecs['pressure_empty_front'] ?? '35' }} PSI</strong><br>
                ...
                <div style="border-top: 1px solid #93c5fd; padding-top: 16px;">
                    ...
                    <ul style="margin: 0; padding-left: 20px; font-size: 14px; color: #1e40af;">
                        ...
                        @endif
                        @if($isElectric)
                        <li>Pressão correta aumenta autonomia elétrica</li>
                        @endif
