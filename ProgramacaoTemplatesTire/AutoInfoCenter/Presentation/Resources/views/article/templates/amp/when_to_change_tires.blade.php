...
<div class="container">
    <article>
        ...
        <div class="symptom-card">
            <div
                class="symptom-card-header {{ $symptom['severity'] === 'alta' ? 'high-severity' : 'medium-severity' }}">
            ...
            <div class="durability-body">
                ...
                <div class="impact-bar">
                    ...
                    <div class="impact-progress">
                        ...
                        @endphp
                        <div class="impact-fill {{ $factor['is_positive'] ? 'positive' : 'negative' }}"
            ...
            <div class="timeline-item">
                ...
                <div class="timeline-content">
                    <div class="timeline-title">
                        {{ $schedule['title'] }}
                        <span
                            class="importance-badge {{ $schedule['importance'] === 'alta' || $schedule['importance'] === 'essencial' || $schedule['importance'] === 'obrigatória' ? 'high' : 'medium' }}">
        ...
        <!-- Sinais Críticos -->
        @if(!empty($article->critical_signs) && is_array($article->critical_signs) && count($article->critical_signs) >
        0)
        ...
        <div class="critical-sign">
            ...
            <div
                style="background-color: #fff; border: 1px solid #fecaca; padding: 12px; border-radius: 8px; margin-top: 12px;">
        ...
        <div class="maintenance-card">
            <div class="maintenance-header">
                ...
                <div class="maintenance-title">Verificação de Pressão</div>
            </div>

            @if(!empty($pressao['frequencia']))
            <div class="maintenance-detail"><strong>Frequência:</strong> {{ $pressao['frequencia'] }}</div>
            @endif

            @if(!empty($pressao['momento']))
            <div class="maintenance-detail"><strong>Quando:</strong> {{ $pressao['momento'] }}</div>
            @endif

            @if(!empty($pressao['tolerancia']))
            <div class="maintenance-detail"><strong>Tolerância:</strong> {{ $pressao['tolerancia'] }}</div>
            @endif

            <div class="maintenance-benefit">
                <strong>Importância:</strong> A pressão correta garante segurança, economia e durabilidade dos pneus.
            </div>
        </div>
        @endif
        ...
        <div class="maintenance-card">
            <div class="maintenance-header">
                <div class="maintenance-icon">🔄</div>
                <div class="maintenance-title">Rodízio de Pneus</div>
            </div>

            @if(!empty($rodizio['frequencia']))
            <div class="maintenance-detail"><strong>Frequência:</strong> {{ $rodizio['frequencia'] }}</div>
            @endif

            @if(!empty($rodizio['padrao']))
            <div class="maintenance-detail"><strong>Padrão:</strong> {{ $rodizio['padrao'] }}</div>
            @endif

            @if(!empty($rodizio['beneficio']))
            <div class="maintenance-benefit">
                <strong>Benefício:</strong> {{ $rodizio['beneficio'] }}
            </div>
            @endif
        </div>
        @endif
        ...
        <div class="maintenance-card">
            <div class="maintenance-header">
                ...
                <div class="maintenance-title">Alinhamento e Balanceamento</div>
            </div>

            @if(!empty($alinhamento['frequencia']))
            <div class="maintenance-detail"><strong>Frequência:</strong> {{ $alinhamento['frequencia'] }}</div>
            @endif

            @if(!empty($alinhamento['sinais']))
            <div class="maintenance-detail"><strong>Sinais:</strong> {{ $alinhamento['sinais'] }}</div>
            @endif

            @if(!empty($alinhamento['importancia']))
            <div
                style="background-color: #fffbeb; border: 1px solid #fed7aa; padding: 8px; border-radius: 6px; font-size: 13px; color: #92400e; margin-top: 8px;">
                <strong>Importância:</strong> {{ $alinhamento['importancia'] }}
            </div>
            @endif
        </div>
        @endif
        ...
        <!-- Cuidados Gerais -->
        @if(!empty($article->preventive_maintenance['cuidados_gerais']) &&
        is_array($article->preventive_maintenance['cuidados_gerais']))
        <div class="care-list">
        ...
        @if(!empty($article->preventive_maintenance['general_care']) &&
        is_array($article->preventive_maintenance['general_care']))
        <div class="care-list">
            <h4>✅ Cuidados Gerais</h4>
            <ul>
            ...
            <div class="specs-grid">
                <div class="specs-section">
                    ...
                    <div class="spec-item">
                        ...
                        <span class="spec-value">{{ translate_vehicle_type($article->vehicle_data['vehicle_category'])
                            }}</span>
                    </div>
                    @endif
                    ...
                    <div class="spec-item">
                        ...
                        <span class="spec-value">{{ translate_vehicle_type($article->vehicle_data['vehicle_type'])
                            }}</span>
                    </div>
                    @endif
                </div>
