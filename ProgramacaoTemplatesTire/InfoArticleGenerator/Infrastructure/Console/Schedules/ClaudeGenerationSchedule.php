...
namespace Src\InfoArticleGenerator\Infrastructure\Console\Schedules;
 ...
 * 1. STANDARD: Executa primeiro (mais econômico, mais frequente)
 * 2. INTERMEDIATE: Escalação para falhas (moderado)
 * 3. PREMIUM: Desabilitado por padrão (muito caro - apenas manual)
 * 
 * ECONOMIA INTELIGENTE:
 * - Standard roda a cada 10 minutos (1 artigo por vez)
 * - Intermediate roda a cada 30 minutos (apenas falhas do standard)
 * - Premium: NUNCA automatizado (só manual)
 * 
 * CONTROLE DE CUSTOS:
 * - Máximo 144 artigos standard/dia (custo: ~144 unidades)
 * - Máximo 48 artigos intermediate/dia (custo: ~110 unidades)
 * - Total diário: ~254 unidades (controlado)
 * 
 * SEGURANÇA:
 ...
 * @version 1.0 - Intelligent Cost-Controlled Generation
 */
class ClaudeGenerationSchedule
{
    ...
    public static function register(Schedule $schedule): void
    {
        ...
        // Gerar com modelo standard - execução frequente
        $schedule->command('temp-article:generate-standard --limit=1 --delay=3')
            ...
            ->before(function () {
                Log::info('Claude Generation: Iniciando geração STANDARD automática');
            })
            ->onSuccess(function () {
        ...
        // Processar falhas do standard com intermediate - menos frequente
        $schedule->command('temp-article:generate-intermediate --only-failed-standard --limit=1 --delay=5')
            ...
            ->before(function () {
                Log::info('Claude Generation: Iniciando escalação INTERMEDIATE automática');
            })
            ->onSuccess(function () {
        ...
        // ⚠️ PREMIUM NUNCA É AUTOMATIZADO (muito caro)
    ...
    private static function performHealthCheck(): void
    {
        try {
            ...
            $totalSuccess = GenerationTempArticle::where('generation_status', 'generated')
                ->orWhere('generation_status', 'validated')
                ->orWhere('generation_status', 'published')
                ->count();
            ...
            if ($stuck > 0) {
                ...
                // Auto-recovery: marcar como failed
    ...
    private static function analyzeDailyCosts(): void
    {
        try {
            ...
            if ($costs['projected_monthly'] > 5000) {
            ...
            $costs['efficiency_score'] = $standardPercentage; // Quanto mais standard, melhor
            ...
            if (!empty($costs['alerts']) && $costs['total'] > 500) {
    ...
    {
        try {
            // Artigos falhados há mais de 30 dias sem retry
            ...
            if ($count > 0) {
                // Marcar como descartados ao invés de deletar
    ...
    private static function checkCostOverrun(): void
    {
        try {
            ...
            // Se custo em 6h > 100 unidades, alertar
            if ($cost6h > 100) {
