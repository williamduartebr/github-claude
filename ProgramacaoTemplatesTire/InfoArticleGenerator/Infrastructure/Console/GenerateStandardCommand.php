 ...
 * 
 * ECONOMIA:
 * - 70-80% dos artigos são resolvidos com este modelo
 * - Custo: 1 unidade por artigo
 * - ROI excelente
 * 
 * USO:
...
class GenerateStandardCommand extends Command
{
    ...
    protected $description = 'Gerar artigos usando modelo STANDARD (claude-3-5-sonnet) - Econômico e Eficiente';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $delay = max((int) $this->option('delay'), 1); // Mínimo 1s
            ...
            // Auto-escalar falhas se configurado
    ...
    private function fetchArticlesToProcess(int $limit)
    {
        ...
        // Incluir retry de falhas do standard (máximo 1 tentativa)
        ...
        return $query->orderByRaw("CASE 
                        WHEN generation_priority = 'high' THEN 1
                        WHEN generation_priority = 'medium' THEN 2
                        WHEN generation_priority = 'low' THEN 3
                        ELSE 4
                    END")
    ...
    private function processArticlesInBatches($articles, int $batchSize, int $delay): void
    {
        ...
        foreach ($batches as $batch) {
            ...
            // Mostrar stats intermediárias a cada lote
    ...
    /**
     * Auto-escalar falhas para intermediate
     */
    private function autoEscalateFailures(): void
    {
        ...
        $failed = GenerationTempArticle::where('generation_status', 'failed')
            ->where('generation_model_used', 'standard')
            ->where('generation_retry_count', 1)
            ->limit(min($this->stats['failed'], 3)) // Máximo 3 escalações automáticas
            ->get();
        ...
        foreach ($failed as $article) {
            ...
            try {
                ...
                sleep(5); // Delay entre escalações
    ...
    private function displayHeader(): void
    {
        ...
        $this->info('║   📊 Claude 3.5 Sonnet (Econômico)                      ║');
        $this->info('║   💰 Custo: 1.0x (base)                                  ║');
    ...
    {
        $estimatedCost = $count * 1.0;
        $estimatedTime = $count * ($delay + 20); // delay + ~20s por artigo

        $this->warn('⚠️ CONFIRMAÇÃO DE EXECUÇÃO:');
        $this->line("📊 Artigos: {$count}");
        $this->line("💰 Custo estimado: {$estimatedCost} unidades");
        $this->line("⏱️ Tempo estimado: " . gmdate("H:i:s", $estimatedTime));
        $this->line("🤖 Modelo: claude-3-5-sonnet-20241022");
